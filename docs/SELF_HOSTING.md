...
```python
from omniai.engine import ModelEngine, EngineSupervisor
...
supervisor = EngineSupervisor(engine)   # watchdog: restart + re-apply LoRA
...
| `RetryPolicy` | **Strategy** | Retry behavior (attempts, backoff, which statuses) is data, swappable per deployment, testable in isolation. |
...
| `EngineSupervisor` | **Watchdog / Supervisor** with **crash-loop breaker** | GPU servers OOM and segfault routinely; recovery (restart → readiness → re-apply LoRA → warm-up) must be automatic, and give-up-after-N prevents thrashing a broken GPU. |
| `on_event` hook, gateway observers | **Observer** | Operational events flow out without coupling the engine to any alerting system. |
...
| Transient 503/timeout during load spikes | Exponential backoff + jitter, only on retryable statuses | `RetryPolicy` |
...
| Server dies at 3 a.m. | Supervisor: health watchdog, restart with backoff, give-up breaker, event hooks for alerting | `EngineSupervisor` |
...
6. **Escape hatches everywhere** — `extra_args`, `env`, `register_backend`,
   injectable `RetryPolicy`/probe/runner seams. The abstraction is a default,
   never a cage.
