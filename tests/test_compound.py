...
async def test_judge_discards_low_quality_pairs():
    ...
    pairs = await judge.curate(
        _logs(
            ("user", "q1"), ("assistant", "good answer"),
            ("user", "q2"), ("assistant", "hallucinated nonsense"),
        )
    )
...
async def test_explicit_feedback_short_circuits_the_judge():
    ...
    pairs = await judge.curate(
        _logs(
            ("user", "q1"), ("assistant", "kept", {"feedback": "thumbs_up"}),
            ("user", "q2"), ("assistant", "dropped", {"feedback": "thumbs_down"}),
        )
    )
...
def test_rehearsal_mix_is_mostly_golden_data(tmp_path):
    ...
    golden.write_text(
        "\n".join(
            json.dumps({"instruction": f"g{i}", "output": f"o{i}"}) for i in range(500)
        )
    )
...
def test_trainer_version_resumes_from_disk(tmp_path):
    (tmp_path / "adapters" / "model-lora-v7").mkdir(parents=True)
    trainer = LoRATrainer("model", tmp_path / "adapters", train_fn=fake_train)
    assert trainer._version == 7  # next adapter will be v8, no collision
...
async def test_shadow_gate_accepts_and_activates(tmp_path):
    ...
    learner = ContinuousLearner(
        buffer, trainer, engine=engine, evaluator=lambda name, path: True
    )
