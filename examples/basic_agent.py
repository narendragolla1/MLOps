...
graph.add_conditional_edges(
    "think", lambda s: "act" if s.messages[-1].content.lstrip().startswith('{"tool"') else END
)
...
router = GatewayRouter(
    handler=graph.compile().as_handler(), interceptors=[PromptGuard()], observers=[buffer]
)
