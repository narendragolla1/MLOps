...
class LoRATrainer:
    ...
    def __init__(
        ...
        **hyperparams: Any,
        ...
        self._version = self._existing_version()

    def _existing_version(self) -> int:
        """Resume version numbering from adapters already on disk, so a
        restart never reuses (and overwrites) an existing adapter name."""
        prefix = f"{Path(self.base_model).name}-lora-v"
        versions = [0]
        if self.output_root.exists():
            for entry in self.output_root.iterdir():
                if entry.name.startswith(prefix) and entry.name[len(prefix):].isdigit():
                    versions.append(int(entry.name[len(prefix):]))
        return max(versions)
...
class ContinuousLearner:
    ...
    evaluator:
        ...
        their weights destroyed (``destroy_rejected``).
    """
    ...
    async def run_cycle(self) -> dict[str, Any]:
        ...
        async with self._lock:
            with traced_span("memory.learning_cycle") as span:
                ...
                if logs:
                    await self.buffer.set_watermark(max(r["created_at"] for r in logs))
    ...
    def trigger(self) -> "asyncio.Task[dict[str, Any]]":
        ...
        and failures are logged instead of vanishing.
        """
